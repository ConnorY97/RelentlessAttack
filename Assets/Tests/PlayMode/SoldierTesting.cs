...
public class SoldierTesting
{
    ...
    public IEnumerator ASoldierMovement_Test()
    {
        ...
        Assert.IsTrue(distance < 1.0f, $"Failed to move towards the target in time, resulting distance: {distance}, cureent position: {mGameObject.transform.position}"); // Check if the enemy moved closer to the target
    }
}
